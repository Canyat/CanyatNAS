    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="glass-card rounded-2xl p-6 shadow-lg space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <div className="p-2 bg-[var(--theme-primary)]/15 text-[var(--theme-primary)] rounded-xl">
              <Palette className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-bold text-sm">主题风格定制 (5 大视觉体系)</h3>
              <p className="text-xs opacity-75">一键切换并实时生效，支持水墨国风、二次元、极简浅色与樱花粉</p>
            </div>
          </div>
            ...
            return (
              <div
                ...
                className={`p-4 rounded-2xl border cursor-pointer transition-all duration-200 relative overflow-hidden flex flex-col justify-between ${
                  isSelected
                    ? 'border-[var(--theme-primary)] bg-[var(--theme-primary)]/15 shadow-lg shadow-[var(--theme-primary)]/20 ring-2 ring-[var(--theme-primary)]/40'
                    : 'glass-inner hover:border-[var(--theme-primary)]/40'
                }`}
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-2.5">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] font-mono opacity-70">{preset.subname}</div>
                    </div>
                  </div>
                  {isSelected && (
                    <span className="p-1 rounded-full bg-[var(--theme-primary)] text-white">
                ...
                <div className="flex items-center justify-between mt-3 pt-2 border-t border-black/10 dark:border-white/10 text-[11px]">
                  <span className="opacity-60">主色调</span>
        ...
        <div className="pt-4 border-t border-black/10 dark:border-white/10 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div className="flex items-center space-x-2">
              ...
              <h4 className="font-semibold text-sm">卡片透明度与磨砂玻璃质感调节</h4>
            </div>

            {/* Quick Presets */}
            <div className="flex items-center space-x-2 text-xs">
              <span className="opacity-60 text-[11px]">快捷质感:</span>
              <button
                type="button"
                onClick={() => {
                  const val = { cardOpacity: 0.25, cardBlur: 20, bgMaskOpacity: 0.4 };
                  themeService.setAppearance(val);
                  setAppearance(val);
                }}
                className="px-2.5 py-1 rounded-lg glass-btn-secondary transition text-[11px]"
              >
                💎 晶莹超透 (25%)
              </button>
              <button
                type="button"
                onClick={() => {
                  const val = { cardOpacity: 0.45, cardBlur: 16, bgMaskOpacity: 0.6 };
                  themeService.setAppearance(val);
                  setAppearance(val);
                }}
                className="px-2.5 py-1 rounded-lg bg-[var(--theme-primary)]/25 text-[var(--theme-primary)] border border-[var(--theme-primary)]/40 transition font-medium text-[11px]"
              >
                ✨ 优雅磨砂 (45%)
              </button>
              <button
                type="button"
                onClick={() => {
                  const val = { cardOpacity: 0.75, cardBlur: 12, bgMaskOpacity: 0.75 };
                  themeService.setAppearance(val);
                  setAppearance(val);
                }}
                className="px-2.5 py-1 rounded-lg glass-btn-secondary transition text-[11px]"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 p-4 glass-inner rounded-2xl text-xs">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-[11px] opacity-60">数值越低卡片越通透，能更清晰地透出背景原画与纹理</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-[11px] opacity-60">调节磨砂玻璃背景的虚化程度，增强文字可读性</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-[11px] opacity-60">调节背景壁纸暗度，兼顾高对比度与艺术视觉</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card rounded-2xl p-6 shadow-lg space-y-4">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm">修改管理员账号用户名</h3>
              <p className="text-xs opacity-75">当前用户名: <strong className="text-[var(--theme-primary)] font-mono">{currentUsername}</strong></p>
            </div>
          </div>

          {usernameMsg && (
            <div className={`p-3 rounded-xl border text-xs flex items-center space-x-2 ${
              usernameMsg.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-rose-500/10 border-rose-500/30 text-rose-400'
            }`}>
          ...
          <form onSubmit={handleChangeUsername} className="space-y-3 text-xs">
            <div>
              <label className="block opacity-75 mb-1">新用户名</label>
              <input
                type="text"
                required
                minLength={2}
                ...
                className="w-full px-3 py-2.5 glass-input rounded-xl font-mono"
              />
            </div>

            <div>
              <label className="block opacity-75 mb-1">验证当前密码</label>
              <input
                type="password"
                required
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-4 py-2 bg-[var(--theme-primary)] hover:opacity-90 text-white text-xs font-semibold rounded-xl transition-all shadow-md disabled:opacity-50"
            >
        ...
        <div className="glass-card rounded-2xl p-6 shadow-lg space-y-4">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm">修改管理员登录密码</h3>
              <p className="text-xs opacity-75">保障面板与 NAS 文件的管理安全</p>
            </div>
          </div>

          {passwordMsg && (
            <div className={`p-3 rounded-xl border text-xs flex items-center space-x-2 ${
              passwordMsg.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-rose-500/10 border-rose-500/30 text-rose-400'
            }`}>
          ...
          <form onSubmit={handleChangePassword} className="space-y-3 text-xs">
            <div>
              <label className="block opacity-75 mb-1">当前旧密码</label>
              <input
                type="password"
                required
                value={oldPassword}
            ...
            <div>
              <label className="block opacity-75 mb-1">新密码 (至少6位)</label>
              <input
                type="password"
                required
                minLength={6}
            ...
            <div>
              <label className="block opacity-75 mb-1">确认新密码</label>
              <input
                type="password"
                required
                minLength={6}
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold rounded-xl transition-all shadow-md disabled:opacity-50"
            >
      ...
      <div className="glass-card rounded-2xl p-6 shadow-lg space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm">系统版本与 GitHub 在线更新</h3>
              <p className="text-xs opacity-75">
                当前运行版本: <span className="text-amber-400 font-mono font-bold">{versionInfo?.currentVersion || 'v1.3.0'}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="flex items-center space-x-1.5 px-3.5 py-2 glass-btn-secondary text-xs font-semibold rounded-xl transition"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center space-x-1 px-3 py-2 glass-btn-secondary text-xs rounded-xl transition"
            >
              <span>GitHub 源码</span>
        ...
        {updateMsg && (
          <div className={`p-3 rounded-xl border text-xs flex items-center justify-between glass-inner ${
            updateMsg.type === 'success' ? 'border-emerald-500/30 text-emerald-400' : 'border-rose-500/30 text-rose-400'
          }`}>
            <div className="flex items-center space-x-2">
            ...
            {versionInfo?.hasUpdate && (
              <button
                ...
                className="px-3 py-1 bg-[var(--theme-primary)] hover:opacity-90 text-white rounded-lg text-xs font-semibold"
              >
      ...
      <div className="glass-card rounded-2xl p-6 shadow-lg space-y-4">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h3 className="font-bold text-sm">NAS 存储挂载点管理</h3>
            <p className="text-xs opacity-75">配置 NAS 文件管理器允许访问的主机目录或外接硬盘挂载路径（支持 Windows 盘符如 C:\、D:\ 与 Linux /media 等）</p>
          </div>
        </div>
            ...
            <div key={root.id} className="p-3.5 flex items-center justify-between text-xs">
              <div className="space-y-0.5">
                <div className="font-semibold flex items-center space-x-2">
                  <span>{root.name}</span>
                  {root.isSystem && (
                    <span className="text-[10px] px-1.5 py-0.2 rounded glass-card opacity-80 font-mono">系统默认</span>
                  )}
                </div>
                <div className="font-mono opacity-70">{root.path}</div>
              </div>
        ...
        <form onSubmit={handleAddRoot} className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
          <input
            ...
            className="px-3.5 py-2.5 text-xs glass-input rounded-xl"
          />
          <input
            type="text"
            ...
            className="px-3.5 py-2.5 text-xs glass-input rounded-xl font-mono"
          />
          <button
            type="submit"
            disabled={isAddingRoot}
            className="px-4 py-2.5 bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold rounded-xl transition-all flex items-center justify-center space-x-1.5 disabled:opacity-50"
          >
      ...
      <div className="glass-card rounded-2xl p-6 shadow-lg space-y-4">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h3 className="font-bold text-sm">WebDAV 网络共享服务</h3>
            <p className="text-xs opacity-75">支持 Windows 映射网络驱动器、Mac Finder 挂载与手机直接连入</p>
          </div>
        </div>

        <form onSubmit={handleSaveSettings} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className="p-4 glass-inner rounded-xl space-y-2">
              <label className="font-semibold block">WebDAV 服务开关</label>
              <div className="flex items-center space-x-3">
                <input
                  type="checkbox"
                  id="webdavToggle"
                ...
                />
                <label htmlFor="webdavToggle" className="cursor-pointer select-none">
            ...
            <div className="p-4 glass-inner rounded-xl space-y-2">
              <label className="font-semibold block">WebDAV 端口</label>
              <input
                type="number"
                value={webdavPort}
                onChange={(e) => setWebdavPort(parseInt(e.target.value, 10))}
                className="w-full px-3 py-2 text-xs glass-input rounded-lg font-mono"
              />
            </div>
          </div>

          <div className="p-4 glass-inner rounded-xl text-xs space-y-2">
            <span className="font-semibold text-[var(--theme-primary)] block">连接指引:</span>
            <ul className="space-y-1 opacity-80 list-disc list-inside leading-relaxed">
              <li><strong>Windows:</strong> 此电脑 ➔ 映射网络驱动器 ➔ 输入 <code className="text-purple-400 font-mono">http://&lt;IP地址&gt;:{webdavPort}/</code></li>
              <li><strong>macOS:</strong> Finder ➔ 前往 ➔ 连接服务器 ➔ 输入 <code className="text-purple-400 font-mono">http://&lt;IP地址&gt;:{webdavPort}/</code></li>
            </ul>
          </div>
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-2.5 bg-[var(--theme-primary)] hover:opacity-90 text-white text-xs font-semibold rounded-xl transition-all shadow-md disabled:opacity-50"
          >
            <Save className="w-3.5 h-3.5" />
