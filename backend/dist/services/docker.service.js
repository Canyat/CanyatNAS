...
class DockerService {
    ...
    async sampleRunningContainersStats(containers) {
        if (this.isSamplingStats || !this.docker)
            return;
...
2026-08-21T01:20:00.000Z [INFO] Scheduled background media library scan completed.`;
    ...
    async streamContainerLogs(id, ws) {
        if (!this.isDockerAvailable || !this.docker) {
            let counter = 1;
    ...
    async execContainerShell(id, ws) {
        if (!this.isDockerAvailable || !this.docker) {
    ...
    async pullImage(imageName, onProgress) {
        if (!this.isDockerAvailable || !this.docker) {
            if (onProgress)
                onProgress(`[Demo Mode] Simulated pulling ${imageName}...`);
            await new Promise(r => setTimeout(r, 1000));
            if (onProgress)
                onProgress(`[Demo Mode] Image ${imageName} pulled successfully!`);
            return true;
        }
    ...
    async deployApp(config) {
        ...
        try {
            ...
            for (const vol of config.volumes) {
                if (vol.hostPath && !fs_1.default.existsSync(vol.hostPath)) {
                    fs_1.default.mkdirSync(vol.hostPath, { recursive: true });
                }
            }
            // Port bindings
            const portBindings = {};
