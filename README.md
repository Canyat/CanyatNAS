...
### 2. 🌐 SMB / CIFS 局域网网络共享中心 (双向共享与挂载)
- **📤 对外共享 NAS 本地硬盘/目录 (SMB 服务端)**：
  - 支持一键将当前 NAS 的任意硬盘分区（如 `D:\`, `E:\`）或指定文件夹（电影、相册、工作文档）发布为局域网 SMB 共享；
  - 自动生成并提供 **Windows 访问路径 (`\\IP\共享名`)** 与 **Mac / iOS / 电视盒子访问路径 (`smb://IP/共享名`)**，配备一键秒级复制；
  - 灵活设置 **「完全控制 (读写)」** 与 **「只读模式」**，支持局域网访客免密匿名访问与权限隔离；
  - 在【NAS 文件管理器】中对任意文件夹支持 **「一键开启 SMB 共享」**。
- **📥 挂载局域网远端设备共享 (SMB 客户端)**：
  - 支持将局域网内的群晖 Synology、TrueNAS、Unraid、Windows 共享主机或软路由 SMB 挂载为本地 NAS 磁盘；
  - **Windows / Linux 智能适配**：Windows 自动推荐分配空闲盘符（如 `Z:`, `Y:`）或直接 UNC 连接；Linux 支持一键 CIFS 自动挂载至 `/mnt` 子目录；
  - **文件管理器无缝联动**：挂载成功的远端 SMB 自动同步至文件管理根目录，支持流媒体在线点播、文本编辑与外链加密分享；
  - **开机自动守护挂载**：支持配置开机/控制面板启动时自动保持远程挂载状态。
